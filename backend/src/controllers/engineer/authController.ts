...
class EngineerAuthController {
  ...
  async demoLogin(req: Request, res: Response) {
    try {
      ...
      // デモアカウントのマッピング（エンジニアアカウント）
      const demoAccounts: Record<string, { email: string; password: string; description: string; name: string }> = {
        engineerA: {
      ...
      const demoAccount = demoAccounts[accountType];
      
      if (!demoAccount) {
        return res.status(400).json({
          success: false,
          message: '無効なデモアカウントタイプです',
        });
      }
      ...
      res.status(200).json({
        success: true,
        message: `デモアカウントでログインしました: ${demoAccount.description}`,
        data: {
          user: {
            id: result.user.id,
            email: result.user.email,
      ...
      // デモアカウントが存在しない場合は作成する
      if (error.message === 'メールアドレスまたはパスワードが正しくありません') {
        return res.status(503).json({
          success: false,
          message: 'デモアカウントが準備されていません。管理者に連絡してください。',
        });
      }
      
      res.status(500).json({
        success: false,
        message: 'デモログイン処理中にエラーが発生しました',
      });
    }
  }
}
