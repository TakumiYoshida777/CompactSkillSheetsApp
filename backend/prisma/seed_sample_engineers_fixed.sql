-- 企業データの作成（SES企業） - Prismaの命名規則に合わせる
...
-- シーケンスのリセット
SELECT setval('companies_id_seq', GREATEST((SELECT MAX(id) FROM companies), 5));
...
-- シーケンスのリセット
SELECT setval('engineers_id_seq', GREATEST((SELECT MAX(id) FROM engineers), 10));
...
-- シーケンスのリセット
SELECT setval('skill_sheets_id_seq', GREATEST((SELECT MAX(id) FROM skill_sheets), 10));
